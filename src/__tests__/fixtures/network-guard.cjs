...
for (const [object, key] of [[net, 'connect'], [net, 'createConnection'], [tls, 'connect']]) {
  const original = object[key];
  object[key] = function (...args) {
    const first = args[0];
    if (typeof first === 'object') { if (!first.path) checked(first.host || first.hostname || 'localhost'); }
    else if (typeof first !== 'string' || /^\d+$/.test(first)) checked(typeof args[1] === 'string' ? args[1] : 'localhost');
